...
def main() -> int:
    ...
    expected = report["register"] and report["version"] == "1.0.0rc3"
    if not args.skip_hermes:
        ...
        with _temporary_directory() as directory:
            ...
            if mode == "core":
                ...
                code = (
                    ...
                    "assert r.content==b'{\"offline\":true}';print('1.0.0rc3')"
                )
            else:
                code = (
                    ...
                    "assert importlib.metadata.version('belief-ledger-gateway')=='1.0.0rc3';"
                    "c=Path(sys.executable).with_name('belief-ledger');assert c.is_file();"
                    "subprocess.run([str(c),'--help'],check=True,capture_output=True,text=True);"
