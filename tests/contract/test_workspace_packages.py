...
def test_core_imports_when_all_hermes_imports_are_rejected(monkeypatch) -> None:
    ...
    assert core.__version__ == "1.0.0rc5"
...
def test_workspace_versions_and_built_constraints_are_synchronized() -> None:
    ...
    assert "belief-ledger-core==1.0.0rc5" in hermes["dependencies"]
    assert "belief-ledger-gateway==1.0.0rc5" in hermes["dependencies"]
    assert "belief-ledger-core==1.0.0rc5" in reference["dependencies"]
    assert "belief-ledger-core==1.0.0rc5" in gateway["dependencies"]
    assert "belief-ledger-core==1.0.0rc5" in mcp["dependencies"]
