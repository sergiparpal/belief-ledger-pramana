...
| [0015 Runtime module layout](0015-runtime-module-layout.md) | accepted, 2026-08-10 | `runtime.py` becomes a package by pure moves, packaged policy data gets one home in core, and the 600-line limit ships as a guard with reasoned per-file ceilings. `EpisodeService` cannot be split by a pure move and stays exempt. |
| [0016 Full-precision recency key](0016-full-precision-recency-key.md) | accepted, 2026-08-19 | `recency_rank` is whole microseconds computed with integer arithmetic, not `int(observed_at.timestamp())`. Second-truncation made saṃśaya depend on where a second boundary fell rather than on the beliefs; the float route to microseconds was rejected because it collapses distinct instants at a rate that grows with the date. |
| [0017 Ablation arms the Suite A instrument cannot isolate](0017-ablation-arms-the-suite-a-instrument-cannot-isolate.md) | accepted, 2026-08-19 | `defeat_only` and `no_gate` were computed from the same `(response, beliefs)` pairs as `flat_baseline` and `full`, so both published an identity as a measured zero. They stay enumerated per specification §10 but carry `measurable: false` and no rate, because `lint_response` cannot observe defeat status or the action gate at any arguments. |
