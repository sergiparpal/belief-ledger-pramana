...
class LedgerStore:
    ...
    def projection_hash(self, version: int = 1) -> str:
        with self.connect() as connection:
            return _projection_hash(connection, version=version)

    def projection_hashes(self) -> dict[str, str]:
        """Return all supported, explicitly versioned projection hashes."""
...
def _projection_hash(connection: sqlite3.Connection, *, version: int = 1) -> str:
    manifests = {1: PROJECTION_MANIFEST_V1, 2: PROJECTION_MANIFEST_V2}
    try:
